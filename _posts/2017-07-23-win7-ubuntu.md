...
1. 打开 DiskGenius，选择一个目标硬盘（比如， `E` 盘）进行分区，点击右键，点击“建立新分区”，新的分区设置将近 70G，对着目标分区（下图 `G` 盘）右键，选择“删除当前分区”，弹出来的窗口中点击“是（Y）”，保持目标分区的“空闲”状态：
   ...
   ![partition](/image/ubuntu_16_04/DiskGenius.png)
   ![create_partition1](/image/ubuntu_16_04/IMG_20170723_160040R.jpg)
   ![create_partition2](/image/ubuntu_16_04/IMG_20170723_160247R.jpg)
   ![target_partition](/image/ubuntu_16_04/target_partition.png)
1. 将下载的 `iso` 系统复制到当前系统的 `E` 盘根目录下，并将 `iso` 中 `casper` 文件夹中的 `vmlinuz.efi` 和 `initrd.lz` 两个文件也复制到 `E` 盘根目录下（这个可以使用 `WinRAR` 打开）,通过点击 DiskGenius 查看 `E` 盘的分区号 `(hd0,5)`:
   ...
   ![hd05](/image/ubuntu_16_04/hd05.png)
1. 打开 `EasyBCD` 软件，依次点击 “添加新条目”、 “NeoGrub”、 “安装”：

   ![neogrup](/imaage/ubuntu_16_04/neogrup.png)
...
1. 进入 Ubuntu 桌面，桌面上有 “Examples” 和 “Install Ubuntu 16.04.2 LTS"，也就是上面 `menu.lst` 中代码的 `title` 行内容：
   ...
   ![desktop](/image/ubuntu_16_04/IMG_20170723_125936R.jpg)
...
1. 安装位置的选择，创建磁盘“分区”（“分区”加引号，因为在 `Linux` 系统中，并没有硬盘分区这个概念，`Linux` 中取而代之的是文件概念，这个和 `Windows` 是有本质区别的）`swap`，`/boot`, `/`: 双击显示“空闲”的分区，弹出“创建分区”对话框，依次填写大小，选择分区类型，分区位置，以及文件系统和挂载点。分区挂载点顺序可以调换，但当有一个设为主分区后其后面的也会跟着默认为主分区，而逻辑分区则不会，建议全部设置为逻辑分区:
   ...
   ![swap](/image/ubuntu_16_04/IMG_20170723_163544R.jpg)
   ![boot](/image/ubuntu_16_04/IMG_20170723_163911R.jpg)
   ![root](/image/ubuntu_16_04/IMG_20170723_164045R.jpg)
1. 在“安装启动引导器的设备”中选择 `/boot` 对应的分区，然后点击“现在安装”：

   ![boot_grub](/image/ubuntu_16_04/IMG_20170723_165118R.jpg)
...
1. 进入 Win 7，打开 `EasyBCD` 软件，依次点击 “添加新条目”、 “NeoGrub”、“删除” ，防止后续误操作重新安装 `Ubuntu`：
   ...
   ![delete_neogrub](/image/ubuntu_16_04/IMG_20170723_171112R.jpg)
1. 点击“Linux/BSD”，然后按下图操作：

   ![add_linux_bsd](/image/ubuntu_16_04/linux_bsd.png)
   ![save](/image/ubuntu_16_04/save.png)
1. 重启，多了一个启动项，选择刚才添加的那个(自己想的名字)，然后会进入以下界面:

   ![start1](/image/ubuntu_16_04/IMG_20170723_171818R.jpg)
   ![start2](/image/ubuntu_16_04/IMG_20170723_204505R.jpg)
